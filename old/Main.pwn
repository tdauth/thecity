...
main()
{
	...
	IRC_CONNECTION = ircConnect("irc.gtanet.com", 6667, "TC_Bot");
	#endif
	*/
	return true;
}
...
public OnPlayerConnect(playerid)
{
    ...
    if (PlayerId != -1)
    {
    	SendMessageToPlayer(playerid, MESSAGE_TYPE_INFO, "Willkommen im Rollenspiel-Spielmodus 'The City'~n~~y~Regeln~n~~w~- Cheats sind nicht erlaubt~n~- Spawn-Kills sind nicht erlaubt~n~- Ungebührliches Verhalten gegenüber anderer Spieler ist nicht erlaubt~n~~y~Hersteller: ~w~Barade");
    	//SendTimerMessageToPlayer(PlayerId, ConvertSeconds(8), MESSAGE_TYPE_INFO, "~y~Anmeldung~n~~w~- Geben Sie '/register [Benutzername] [Passwort] [Email-Addresse] [Landeskürzel]' im Chat ein, um sich zu registrieren.~n~- Falls Sie bereits ein Benutzerkonto besitzen, geben Sie '/login [Benutzername] [Passwort]' im Chat ein, um sich anzumelden.");
...
public OnPlayerText(playerid, text[])
{
    ...
    #if defined a_irc_used
    new InfoMessage[MAX_STRING];
	format(InfoMessage, sizeof(InfoMessage), "%s: %s", PLAYER_NAME[PlayerId], Text);
...
/*
public OnPlayerTeamPrivMsg(playerid, Message[])
{
    print("On Player Team Priv Msg");
	new SaveText[MAX_STRING];
	format(SaveText, sizeof(SaveText), " zu zu seinem Team: %s", Message);
    SavePlayerChatLogData(playerid, SaveText);
	return true;
}
*/
		...
		case MENU_TYPE_HOUSES:
  		{
		  	...
		  	CreateHouseSettingsMenu(playerid, ACCOUNT_P_HOUSE[AccountId][Accountplayerid][row]);
			return true;
  		}
		//House Settings Menu
...
{
    print("On Vehicle Mod");
    return true;
}

public OnVehiclePaintjob(vehicleid, paintjobid)
{
    print("On Vehicle Paintjob");
    return true;
}

public OnVehicleRespray(vehicleid, color1, color2)
{
    print("On Vehicle Respray");
    return true;
}
...
/*
#if defined a_irc_used
public ircOnConnect(Connection)
{
    print("irc On Connect");
    ircJoinChan(Connection, "#GTA_SA_MP_TC");
    InitIRCTimer();
    IRC_CONNECTIONS++;
    ircSendRawData(Connection, "PRIVMSG NickServ :IDENTIFY asd");
    SendClientMessageToAllPlayers(MESSAGE_TYPE_SERVER, "Verbindung mit IRC-Channel wurde aufgebaut.");
    return true;
}

public ircOnUserSay(Connection, Channel[], User[], Message[])
{
    print("irc On User Say");
	new InfoMessage[MAX_STRING];
	format(InfoMessage, sizeof(InfoMessage), "IRC - %s - %s: %s", Channel, User, Message);
    SendClientMessageToAllPlayers(MESSAGE_TYPE_SERVER, InfoMessage);
    return true;
}

public ircOnUserPart(Connection, Channel[], User[])
{
    print("irc On User Part");
    new InfoMessage[MAX_STRING];
	format(InfoMessage, sizeof(InfoMessage), "IRC - %s: %s", Channel, User);
    SendClientMessageToAllPlayers(MESSAGE_TYPE_SERVER, InfoMessage);
    return true;
}

public ircOnUserJoin(Connection, Channel[], User[])
{
    print("irc On User Join");
    new InfoMessage[MAX_STRING];
	format(InfoMessage, sizeof(InfoMessage), "IRC - %s ist dem Channel %s beigetreten.", User, Channel);
    SendClientMessageToAllPlayers(MESSAGE_TYPE_SERVER, InfoMessage);
    return true;
}
#endif
